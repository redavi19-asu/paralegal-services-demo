    ...
    <section id="story-scales" className="relative"><div ref={ref} className="h-[220vh]"><div className="sticky top-16 h-[calc(100vh-4rem)] flex items-center"><Container>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center"><div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href="#appointments" className="inline-flex items-center gap-2 rounded-xl border px-5 py-3 text-sm font-semibold bg-foreground text-background hover:opacity-90"><CalendarIcon className="h-4 w-4" /> Book Now</a>
          <a href="#services" className="rounded-xl border px-5 py-3 text-sm font-medium hover:bg-muted">See services</a>
        </div>
      </div><div className="relative">
        <motion.svg viewBox="0 0 600 500" className="w-full h-[55vh]"><defs><linearGradient id="gradLaw" x1="0" x2="1" y1="0" y2="1"><stop offset="0%" stopColor="#6366f1" /><stop offset="100%" stopColor="#22d3ee" /></linearGradient></defs><motion.path d="M300 60 L300 130 M300 130 L160 180 M300 130 L440 180 M160 180 L120 300 M200 300 L160 180 M120 300 A50 20 0 1 0 200 300 M440 180 L400 300 M480 300 L440 180 M400 300 A50 20 0 1 0 480 300 M300 130 L300 420 M240 420 L360 420" fill="none" stroke="url(#gradLaw)" strokeWidth="8" strokeLinecap="round" style={{ pathLength: pathProgress }} /><motion.path d="M60 460 L540 460" fill="none" stroke="#94a3b8" strokeWidth="6" strokeLinecap="round" style={{ pathLength: sublineProgress }} /></motion.svg>
...
function ScenePhone() {
  ...
  return (
    <section id="story-phone" className="relative"><div ref={ref} className="h-[260vh]"><div className="sticky top-16 h-[calc(100vh-4rem)]"><Container className="h-full grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
      <div className="relative py-8"><h2 className="text-3xl md:text-4xl font-semibold">A calm, guided process</h2><p className="mt-3 text-muted-foreground max-w-prose">From intake to filing, Candice keeps every document and deadline in check.</p><div className="mt-6 space-y-6">{steps.map((step) => <motion.div key={step.title} style={{ opacity: step.animation }} className="flex items-start gap-3"><div className="mt-1 h-2.5 w-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-cyan-400" /><div><div className="font-medium">{step.title}</div><div className="text-sm text-muted-foreground">Clear checklists, versioned docs, and reminders.</div></div></motion.div>)}</div></div>
      <div className="flex items-center justify-center"><motion.div className="relative w-[270px] md:w-[320px] aspect-[9/19.5] rounded-[36px] border bg-card shadow-2xl overflow-hidden" style={{ opacity: phoneOpacity, scale: phoneScale }}><div className="absolute left-1/2 -translate-x-1/2 top-0 mt-2 h-6 w-40 rounded-full bg-background/60 border" /><motion.div className="absolute inset-0 p-4 space-y-3" style={{ y: cardY, opacity: cardOpacity }}><div className="h-10 rounded-xl bg-gradient-to-r from-indigo-500/30 to-cyan-500/30 border flex items-center px-3 text-xs">Client Portal</div><div className="rounded-xl border bg-muted/40 p-3"><div className="text-xs text-muted-foreground">Upcoming Deadline</div><div className="text-sm font-medium">Smith v. Smith — Response due Oct 30</div></div><div className="grid grid-cols-2 gap-3"><div className="h-24 rounded-xl border bg-muted/40 p-3 text-xs">Intake Packet.pdf</div><div className="h-24 rounded-xl border bg-muted/40 p-3 text-xs">Draft Petition.docx</div></div><div className="h-12 rounded-xl border bg-muted/40 p-3 text-xs">Message Candice…</div></motion.div></motion.div></div>
    </Container></div></div></section>
  );
}
...
function AppointmentScheduler() {
  ...
  const openCalendar = () => {
    ...
    else { dateRef.current?.focus(); dateRef.current?.click(); }
  };
  const today = new Date().toISOString().split("T")[0];

  return (
    <section id="appointments" className="py-16 lg:py-24 border-t bg-muted/20"><Container>
      <div className="text-center max-w-3xl mx-auto mb-10"><Pill>Appointments</Pill><h2 className="mt-3 text-3xl md:text-4xl font-semibold tracking-tight">Book your consultation</h2><p className="mt-3 text-muted-foreground">Choose your date, time, and meeting type. Tap the calendar button to pick a date.</p></div>
      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_.9fr] gap-8 items-start"><div className="rounded-3xl border bg-card p-6 md:p-8 shadow-sm">
        {!submitted ? <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="relative"><input ref={dateRef} required name="date" value={form.date} onChange={update} min={today} className="w-full rounded-xl border px-4 py-3 pr-12 bg-background" type="date" /><button type="button" onClick={openCalendar} aria-label="Open calendar" className="absolute right-2 top-1/2 -translate-y-1/2 h-9 w-9 rounded-lg border bg-card grid place-items-center hover:bg-muted"><CalendarIcon className="h-5 w-5" /></button></div>
          ...
          <button className="md:col-span-2 inline-flex items-center justify-center gap-2 rounded-xl border px-5 py-3 text-sm font-semibold bg-foreground text-background"><CalendarIcon className="h-4 w-4" /> Submit appointment request</button>
